window.ZORIX_STATUS = {
  "updatedAt": "2026-09-09T20:52:39+02:00",
  "overall": "partial_outage",
  "message": "Some Zorix systems are currently experiencing issues, including Nex Coder 3.8 availability for affected user groups.",
  "components": [
    {
      "id": "zorix-api",
      "name": "Zorix API",
      "description": "Core API requests and model routing.",
      "status": "maintenance",
      "statusMessage": "It's working at maintenance for secure events"
    },
    {
      "id": "zorix-code",
      "name": "Zorix Code",
      "description": "Coding environment, tools and agent workflows.",
      "status": "operational"
    },
    {
      "id": "metron",
      "name": "Zorix Metron",
      "description": "Usage, models, benchmarks, voting and public data.",
      "status": "operational"
    },
    {
      "id": "nex",
      "name": "Nex models",
      "description": "Nex and Nex Coder model availability.",
      "status": "partial_outage",
      "statusMessage": "Since 17:23 CEST on Sep 9, some SovreIgn, regular, Free, Nex, Core and Apex users may be unable to use Nex Coder 3.8."
    },
    {
      "id": "virexa",
      "name": "VIREXA",
      "description": "Image generation systems.",
      "status": "degraded"
    },
    {
      "id": "public-quota",
      "name": "Public quota",
      "description": "Shared public token capacity and quota accounting.",
      "status": "partial_outage"
    },
    {
      "id": "updates",
      "name": "Updates & documentation",
      "description": "Release notes, documentation and status pages.",
      "status": "operational"
    }
  ],
  "incidents": []
};
